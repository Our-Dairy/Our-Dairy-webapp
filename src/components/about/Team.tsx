...
const TeamCard = ({ member }: { member: TeamMember }) => (
    <div className="group flex flex-col items-center py-8 text-sm bg-white border border-gray-300/60 w-64 rounded-md cursor-pointer hover:border-gray-400 hover:shadow-lg transition">
        <img className="w-24 rounded-full" src={member.image} alt={member.name} />
        <h2 className="text-gray-700 text-lg font-medium mt-2">{member.name}</h2>
        <p className="text-gray-500">{member.role}</p>
        <p className="text-center text-gray-500/60 w-3/4 mt-4">{member.description}</p>
...
export default function Team() {
    return (
        <div className="flex flex-col items-center text-center">
            <h3 className="text-lg font-medium text-green-600 mb-2">Contact Us</h3>
            <h1 className="text-3xl md:text-4xl font-semibold mb-4 text-green-400">Meet Our People</h1>
            <p className="w-3/5 mb-14 text-gray-500 text-sm">
                Our dedicated team is committed to revolutionizing dairy management through innovative technology and user-centric design.
            </p>
